...
function defaultId() {
  return 'mqttts_' + Math.random().toString(16).substr(2, 8);
}

function sendPacket(client, packet, cb?) {
  client.emit('packetsend', packet);

  let result = mqttPacket.writeToStream(packet, client.stream);
...
export class MqttClient extends events.EventEmitter {
  ...
  constructor(public streamBuilder: (client: MqttClient) => IStream, options: IClientOptions) {
    super();
    let k;
    let that = this;
  ...
  private _setupStream() {
    ...
    let parser = mqttPacket.parser(this.options);
  ...
  private _checkDisconnecting(callback) {
   ...
   */
  public publish(topic, message, opts?, callback?) {
    let packet;
    ...
    // default opts
    let defaultOpts = { qos: 0, retain: false, dup: false };
  ...
  public subscribe(topic: string | string[], opts?: IClientSubscribeOptions, callback?: ClientSubscribeCallback): this {
    ...
    let subs = [];
    let obj = args.shift();
    let resubscribe = obj.resubscribe;
    ...
    invalidTopic = validateTopics(obj);
    ...
    let defaultOpts = { qos: 0 };
    opts = xtend(defaultOpts, opts);

    if (Array.isArray(obj)) {
      obj.forEach(function (_topic) {
        if (that._resubscribeTopics[_topic] < opts.qos ||
          !that._resubscribeTopics.hasOwnProperty(_topic) ||
          resubscribe) {
          subs.push({
            topic: _topic,
            qos: opts.qos
    ...
    this.outgoing[packet.messageId] = function (err, outgoingPacket) {
      if (!err) {
        let granted = outgoingPacket.granted;
  ...
  public unsubscribe(topic: string | string[], callback?: PacketCallback): this {
    ...
    if (this.options.resubscribe) {
      packet.unsubscriptions.forEach(function (unsubscribeTopic) {
        delete that._resubscribeTopics[unsubscribeTopic];
      });
    }
  ...
  private _handleConnack(packet) {
    ...
    if (rc === 0) {
      ...
      let err = new Error(`Code: ${rc} Connection refused: ${errors[rc]}`);
      this.emit('error', err);
    }
  }
  ...
  */
    ...
    switch (type) {
      ...
      case 'suback':
        ...
        if (packet.granted.length === 1 && (packet.granted[0] & 0x80) !== 0) {
          ...
          let topics = this.messageIdToTopic[mid];
          if (topics) {
   ...
   * @api private
   */
  private _handlePubrel(packet: Packet, callback) {
    let mid = packet.messageId;
    let that = this;

    let comp = { cmd: 'pubcomp', messageId: mid };

    that.incomingStore.get(packet, function (storeErr, pub) {
      if (!storeErr && pub.cmd !== 'pubrel') {
        that.emit('message', pub.topic, pub.payload, pub);
        that.incomingStore.put(packet);
        that.handleMessage(pub, function (messageErr) {
          if (messageErr) {
            return callback && callback(messageErr);
          }
